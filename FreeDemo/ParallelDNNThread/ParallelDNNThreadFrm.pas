...
uses
  ...
  Geometry2DUnit, MemoryRaster, zDrawEngine, zDrawEngineInterface_SlowFMX, PictureViewerInterface,
  zAI_Common, zAI, FMX.Objects, FMX.StdCtrls, FMX.Memo.Types;
    ...
    begin
      for i := 0 to ViewIntf.Count - 1 do
        // 异步方法是把数据发送给DNN线程,待它处理完成后,返回事件,该方法不产生延迟,符合HPC机制
          TAI_DNN_Thread_MMOD6L(DNNPool.MinLoad_DNN_Thread).ProcessP(@Face_Arry[i], ViewIntf[i].raster, False,
          procedure(ThSender: TAI_DNN_Thread_MMOD6L; UserData: Pointer; Input: TRaster; output: TMMOD_Desc)
          var
            p: PFaceData;
            j: Integer;
          begin
    ...
    begin
      ParallelFor(0, ViewIntf.Count - 1,
        ...
        var
          ...
          j: Integer;
        begin
          with DNNPool.MinLoad_DNN_Thread as TAI_DNN_Thread_MMOD6L do
            begin
