...
uses SysUtils, Windows, ShellAPI,
  CoreClasses, PascalStrings, UnicodeMixedLib, DoStatusIO, ListEngine,
  MemoryRaster, MemoryStream64, Learn, LearnTypes,
  zAI, zAI_Common, zAI_KeyIO, zAI_TrainingTask;
...
begin
  ...
  ai := TAI.OpenEngine(Prepare_AI_Engine(aiEng));
...
begin
  ...
  if keep <> '' then
      zAI.KeepPerformanceOnTraining := umlStrToInt(keep, 0);
