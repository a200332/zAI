...
uses
  ...
  MemoryRaster, zDrawEngine, zDrawEngineInterface_SlowFMX, Geometry2DUnit, PictureViewerInterface;
...
function BuildDemoPicture(var angle: TGeoFloat; deltaTime: Double): TRaster;
...
begin
  ...
  with Result.DrawEngine do
    begin
      for i := 0 to Rasterization.TextCoordinate.Count - 1 do
        begin
          DrawBox(Rasterization.TextCoordinate[i].BoundBox, DEColor(1, 0, 0), 1);
          DrawBox(Rasterization.TextCoordinate[i].BoundBox.BoundRect, DEColor(1, 1, 1), 1);
        end;
      Flush;
    end;
end;
...
begin
  ...
  FillBlackGrayBackgroundTexture(viewIntf.BackgroundTex, 32, RColorF(0.9, 0.9, 0.9), RColorF(0.8, 0.8, 0.8), RColorF(0.85, 0.85, 0.85));
  angle := 0;
