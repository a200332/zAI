    ...
    begin
      ...
      try
        ...
        if not umlFileExists(bear_od_file) then
          begin
            ...
            *)
            ...
            // 我取的值很大，因此，训练完成这个熊本熊的数据集，需要15分钟左右
        ...
        if umlFileExists(bear_od_file) then
          begin
            DoStatus('载入熊本熊的OD3L检测器 : %s', [bear_od_file.Text]);
            ...
            TThread.Synchronize(TThread.CurrentThread, procedure
              begin
                ...
                DoStatus('干扰GPU-性能测试：正在做GPU-OD3L性能测试,5秒后将会报告测试结果.');
                tk := GetTimeTick();
                ...
                DoStatus('干扰GPU-性能测试：GPU-OD3L在5秒内总共算成了 %d 次OD3L检测，大约每秒完成 %d 次检测', [mmod_processcounter, Round(mmod_processcounter / 5.0)]);
              end);
            ...
            TThread.Synchronize(TThread.CurrentThread, procedure
              begin
                ...
                DoStatus('纯净GPU-性能测试：正在做GPU-OD3L性能测试,5秒后将会报告测试结果.');
                tk := GetTimeTick();
                mmod_processcounter := 0;
                while GetTimeTick() - tk < 5000 do
                  begin
                ...
                DoStatus('纯净GPU-性能测试：GPU-OD3L在5秒内总共算成了 %d 次OD3L检测，大约每秒完成 %d 次检测', [mmod_processcounter, Round(mmod_processcounter / 5.0)]);
              end);
