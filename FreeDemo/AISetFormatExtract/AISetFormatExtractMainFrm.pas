    ...
    begin
      ...
      for i := 0 to editor_dataset.Count - 1 do
        begin
          ...
          DoStatus('正在解析 %s 中的数据', [editor_ImgData.FileInfo.Text]);

          // 原始光栅数据
          editor_ImgData_raster := editor_ImgData.Raster;
          DoStatus(' %s 原始的光栅尺寸 %d * %d', [editor_ImgData.FileInfo.Text, editor_ImgData_raster.Width, editor_ImgData_raster.Height]);
          ...
          for j := 0 to editor_ImgData.DetectorDefineList.Count - 1 do
            begin
              ...
              DoStatus(' %s 中的检测器框体%d: %d %d %d %d shapePredictor数据有 %d 个', [editor_ImgData.FileInfo.Text, j,
          ...
          for j := 0 to editor_ImgData.GeometryList.Count - 1 do
            begin
              ...
              DoStatus(' %s 中的几何描述体 %s 有 %d 个塌陷 ', [editor_ImgData.FileInfo.Text, editor_ImgData_Geo.Token.Text, editor_ImgData_Geo.CollapsesCount]);
            end;
          ...
          for j := 0 to editor_ImgData.SegmentationMaskList.Count - 1 do
            begin
              ...
              DoStatus(' %s 中有一个叫 %s 的分割蒙版描述对象 ', [editor_ImgData.FileInfo.Text, editor_ImgData_SegMask.Token.Text]);
            end;
