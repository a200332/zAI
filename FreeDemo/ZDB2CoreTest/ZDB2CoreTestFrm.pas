...
begin
  ...
  ZDB.NewStream(ZDBMem, 1024 * 1024 * 64, 8 * 1024, TZDB2_Core_SpaceMode.stmBigData);
  DoStatus('New DB Done. size:%s physics block:%d', [umlSizeToStr(ZDB.Space.Space_IOHnd^.Size).Text, ZDB.Space.PhyBlockNum]);
...
begin
  ZDB.TraversalP(False, False, nil, procedure(Sender: TZDB2; Traversal: TZDB2_Traversal; Mem: TMem64; var Running: Boolean)
    var
      n: TMem64;
    begin
      n := TMem64.Create;
      ...
      PostThreadPool.MinLoad_Thread.PostP3(nil, n, Traversal.Current, procedure(Data1: Pointer; Data2: TCoreClassObject; Data3: Variant)
        begin
          DoStatus('条目:%d md5:%s crc32:%8x', [Integer(Data3), umlMD5String(TMem64(Data2).Memory, TMem64(Data2).Size).UpperText,
            umlCRC32(TMem64(Data2).Memory, TMem64(Data2).Size)]);
          DisposeObject(Data2);
        end);
    end, nil);
end;
...
begin
  ...
  ZDB.PostP(Mem, True, nil, procedure(Sender: TZDB2; UserData: Pointer; ID: Integer; Successed: Boolean)
    begin
      if Successed then
          DoStatus('写入成功 ID:%d size:%d', [ID, Sender.Space.GetDataSize(Sender.Space.PhyBlock[ID].ID)]);
    end);
end;
...
begin
  ...
  // 将并行处理数据画出来
  f := round(sqrt(ZDB.Space.PhyBlockNum)); // 以平方根方式切割预览数据
  num := 0;
  X := 0;
  Y := 0;
  box := RectV2(X, Y, X, Y);
    ...
    begin
      for i := 0 to f do
        begin
          ...
          if num < ZDB.Space.PhyBlockNum then
            begin
              if Vec2InRect(mouse_pt, d.SceneToScreen(r)) then
                begin
                  p := @ZDB.Space.PhyBlock[num];
    ...
    begin
      for i := 0 to f do
        begin
          r := RectV2(X, Y, X + c_Metric, Y + c_Metric);
          if num < ZDB.Space.PhyBlockNum then
            with ZDB.Space.PhyBlock[num] do
              begin
