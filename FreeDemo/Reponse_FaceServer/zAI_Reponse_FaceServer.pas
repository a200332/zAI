...
begin
  ...
  // 使用人脸对齐工具
  if Scale4x then
      Alignment := TAlignment_Face.Create(ai) // TAlignment_Face 会将照片放大4倍后做人脸对齐，它返回的坐标系是原始坐标系
  else
      Alignment := TAlignment_FastFace.Create(ai); // TAlignment_FastFace 不会对照片做任何放大处理，它返回的坐标系是原始坐标系
...
begin
  ...
  if imgL <> nil then
    begin
      ...
      m64 := TMemoryStream64.Create;
      imgL.SaveToStream(m64, True, True, TRasterSaveFormat.rsJPEG_YCbCr_Qualily90);
