...
uses
  ...
  ZIOThread,
  Geometry2DUnit, zDrawEngine, zDrawEngineInterface_SlowFMX;
...
begin
  ...
  TCompute.RunP_NP(procedure
    var
      PostPool: TPost_ThreadPool;
      InputDone: TAtomBool;
    begin
      // 在pool创建10个执行线程
      PostPool := TPost_ThreadPool.Create(10);
  ...
  TCompute.RunP_NP(procedure
    ...
    begin
        ...
        begin
          for i := 0 to length(StateArry) - 1 do
            begin
              // 将数据编入IO线程的处理队列
              // IOThread编队机制是多线程安全的
              IOThread.EnQueueP(TIOData.Create, @StateArry[i],
                procedure(thSender: TIOData)
                var
                  p: PState;
                begin
                  MT19937Randomize();
      ...
      while IOThread.Count > 0 do
        with IOThread.DeQueue(True) do
          begin
            PState(Data)^.picked := True;
            Free;
          end;
...
begin
  ...
  DrawPool(pb).PostScrollText(60 * 60 * 1000, '输入输出线程叫IOThread,程序库:ZIOThread.pas', 18, DEColor(1, 1, 1, 1));
