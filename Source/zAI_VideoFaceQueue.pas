...
begin
  ...
  if length(p^.MMOD) > 0 then
    begin
      AI := Owner.FParallel.GetAndLockAI;
      ...
      for i := 0 to AI.Face_chips_num(fHnd) - 1 do
        begin
          ...
          with TAI_DNN_Thread_Metric(Owner.DNN_Face_Metric_Pool.MinLoad_DNN_Thread) do
              ProcessM(@p^.Metric[i], p^.Metric[i].Raster, False, {$IFDEF FPC}@{$ENDIF FPC}FaceMetric_OnResult);
        end;
      AI.Face_Close(fHnd);
      Owner.FParallel.UnLockAI(AI);
    end
...
begin
  ...
  i := Owner.FLearn.ProcessMaxIndex(p^.Data);
  p^.K := LDistance(p^.Data, Owner.FLearn[i]^.m_in);
  p^.Token := Owner.FLearn[i]^.Token;
  p^.Done := True;
  if Assigned(OnInterface) then
...
begin
  ...
  with TAI_DNN_Thread_MMOD6L(Owner.DNN_Face_Detector_Pool.MinLoad_DNN_Thread) do
