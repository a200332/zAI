...
type
  ...
  private
    ...
    FExtractFaceWidth, FExtractFaceHeight: Integer;
  ...
  public
    ...
    property ExtractFaceWidth: Integer read FExtractFaceWidth write FExtractFaceWidth;
    property ExtractFaceHeight: Integer read FExtractFaceHeight write FExtractFaceHeight;
...
begin
  ...
  fHnd := AI.Face_Detector(Input, output, FExtractFaceWidth);
      ...
      begin
        ...
        if (cent >= 0) and RectInRect(output[cent].R, Input.BoundsRectV2) then
          begin
            ...
            det := img.DetectorDefineList.AddDetector(Rect2Rect(RectProjection(Input.BoundsRectV2, img.Raster.BoundsRectV2, output[cent].R)), p^.FaceToken);
      ...
      begin
        ...
        for i := 0 to AI.Face_chips_num(fHnd) - 1 do
          if RectInRect(output[i].R, Input.BoundsRectV2) then
            begin
              det := img.DetectorDefineList.AddDetector(Rect2Rect(RectProjection(Input.BoundsRectV2, img.Raster.BoundsRectV2, output[i].R)), p^.FaceToken);
...
begin
  ...
  FDNN_Pool.BuildPerDeviceThread(1, TAI_DNN_Thread_MMOD6L);
  ...
  FParallel.Prepare_Face;
  FExtractFaceWidth := C_Metric_Input_Size;
  FExtractFaceHeight := C_Metric_Input_Size;
